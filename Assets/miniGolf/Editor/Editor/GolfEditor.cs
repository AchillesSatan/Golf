...
public class GolfEditor : EditorWindow {
	...
	void createNewScenes()
	{
		...
		for(int i=0; i<18; i++)
		{
			...
			GameObject go1 = (GameObject)Resources.LoadAssetAtPath(prefabName, typeof(GameObject));
	...
	{
		GameObject go = (GameObject)Resources.LoadAssetAtPath(getPath(i,false), typeof(GameObject));
		return go;
	}
	...
	public GameObject createObject(Object obj, bool raycastDown=false)
	{
		...
		{
			Camera sceneCamera = SceneView.lastActiveSceneView.camera;
			Vector3 cameraPos = sceneCamera.camera.ScreenToWorldPoint(new Vector3(sceneCamera.pixelWidth/2f, sceneCamera.pixelHeight/2f, 0f));
			cameraPos.y = 0;
