...
public class BallScript : MonoBehaviour 
{
	...
	void init()
	{
		...
		if(rigidbody && rigidbody.collider)
		{
			m_initalMaterial = rigidbody.collider.material;
		}	
	...
	public void Start()
	{
		...
		m_initalLinearDrag = rigidbody.drag;
		m_initalAngularDrag = rigidbody.angularDrag;
		//set the inital mode
	...
	public void OnCollisionEnter(Collision ci)
	{
		...
		{
			rigidbody.angularVelocity = rigidbody.angularVelocity * 0.65f;
	...
	public void LateUpdate()
	{
		...
		if(m_count==0)
		{
			m_prevDir = rigidbody.velocity;
			m_dir = rigidbody.velocity;
		}
	                   ...
	                   PhysicMaterial hillMAT)
	{
		...
		if(rigidbody && rigidbody.collider)
		{
			rigidbody.collider.material = mat;
		}
	...
	{
		float angVelocity = rigidbody.angularVelocity.magnitude;
	...
	{
		if(m_trail && m_trail.renderer)
		{
			m_trail.renderer.enabled=enabled;
		}				
	}
	...
	void fireBall(float currentForce)
	{
		...
		rigidbody.AddForce( vec *  currentForce  ) ;
		if(m_gameScript)
		{
		...
		rigidbody.drag = m_initalLinearDrag;
		//set the balls drag back to the inital angular drag
		rigidbody.angularDrag = m_initalAngularDrag;
		m_slowDown=false;
		//reset our roll time
		m_rollTime = 0f;
